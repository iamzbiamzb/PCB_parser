...
while line:
	if line.find('( layer "')!=-1:
		...
		if line.find('DIELECTRIC')==-1 and line.find('SURFACE')==-1:
			if line.find('TOP')!=-1 or line.find('S')!=-1 or line.find('BOTTOM')!=-1:
...
while line:
	if line.find('netClass "')!=-1:	  
		...
		while line.find('member (') == -1 and line.find('objectStatus') == -1:
...
for i,j in chipDict.items():
	...
	gds_layer = -1;
	if chipLayerDict.get(chip_name, 'noExist') != 'noExist':
		layer = chipLayerDict[chip_name]
		if layerDict.get(layer, 'noExist') != 'noExist':
			gds_layer = layerDict[layer]

	if UsedPinDict.get(i, 'noExist') != 'noExist':
		print("PinPad : " + i)
		print("\t Layer : " + str(gds_layer))
		#print("\t Layer : " + chipLayerDict[chip_name])
		print("\t Coor : " + j)
		print("\t Type : " + chip_type)
		print("\t\t Width : " + str(width))
		print("\t\t Height : " + str(height))
...
print("MAX_LAYER: " + str(len(layerDict)))
print(layerDict)
